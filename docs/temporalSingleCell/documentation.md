...
A problem is that the flux rates change with timepoints, and for now the networks used are static for the single celltype, so they have the same weight throughout the simulation. There is no way to change the weights of the edges in the graph in this moment, maybe in the future when different time points are considered for every single timepoint.
