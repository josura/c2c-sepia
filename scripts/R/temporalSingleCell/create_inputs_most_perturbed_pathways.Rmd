...
for (cellType in cellTypes){
  nodes <- nodesinfo[nodesinfo$Name %in% unique(inputGraphs[[cellType]]$Source) | nodesinfo$Name %in% unique(inputGraphs[[cellType]]$Target),]
  write.table(nodes, file = paste0("/home/josura/Projects/ccc/datiIdo/inputGraphs/1h/nodesWithLR/",cellType,".tsv"), sep = "\t", row.names = FALSE,quote = FALSE)
}
```
...
df.net <- subsetCommunication(cellchat)
# save the inferred cellular communication network as an object
saveRDS(df.net, file = "inputGraphs/1h/cellchat_inferred_network.1h.withLR.rds")
```
...
```{r}
# load the inferred cellular communication network
df.net <- readRDS("inputGraphs/1h/cellchat_inferred_network.1h.withLR.rds")
```
