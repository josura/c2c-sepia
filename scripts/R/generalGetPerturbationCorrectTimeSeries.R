...
getperturbationTimeSeries <- function(path,output){
  ...
  for (typeName in type_list){
    ...
    write(paste(c("iteration","time",node_list),collapse = "\t"), file = outputFileName, append = FALSE, sep = "\t")
  }
  ...
  for (file_name in file_list) {
    ...
    if (endsWith(file_name, ".tsv")) {
      ...
      timeValue <- df$time[1] # the time value is the same for all the values in the file
      write(paste(c(file_suffix,timeValue,colValues),collapse = "\t"), file = outputFileName, append = TRUE, sep = "\t")
